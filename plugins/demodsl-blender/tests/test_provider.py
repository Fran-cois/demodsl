"""Tests for demodsl-blender plugin — bridge, provider, device manifest."""
...
# Path to the device manifest bundled with the plugin
_MANIFEST_PATH = (
...
class TestDeviceRenderingNewFields:
    ...
    def test_camera_distance_bounds(self) -> None:
        with pytest.raises(ValidationError):
            DeviceRendering(camera_distance=0)
        with pytest.raises(ValidationError):
            DeviceRendering(camera_distance=11)

    def test_rotation_speed_bounds(self) -> None:
        with pytest.raises(ValidationError):
            DeviceRendering(rotation_speed=0)
        with pytest.raises(ValidationError):
            DeviceRendering(rotation_speed=6)
    ...
    def test_dof_aperture_bounds(self) -> None:
        with pytest.raises(ValidationError):
            DeviceRendering(dof_aperture=0)
        with pytest.raises(ValidationError):
            DeviceRendering(dof_aperture=23)

    def test_film_grain_bounds(self) -> None:
        with pytest.raises(ValidationError):
            DeviceRendering(film_grain=-0.1)
        with pytest.raises(ValidationError):
            DeviceRendering(film_grain=1.1)
...
class TestCheckBlenderAvailable:
    def test_blender_missing(self) -> None:
        with patch("demodsl_blender.bridge._find_blender", return_value=None):
            assert check_blender_available() is False

    def test_script_missing(self) -> None:
        with (
            patch(
                "demodsl_blender.bridge.shutil.which",
                return_value="/usr/bin/blender",
            ),
            patch(
                "demodsl_blender.bridge._BLENDER_DIR",
                Path("/nonexistent"),
            ),
        ):
            assert check_blender_available() is False
    ...
    def test_all_present(self, tmp_path: Path) -> None:
        ...
        with (
            ...
            patch("demodsl_blender.bridge._BLENDER_DIR", tmp_path),
        ):
...
class TestRenderViaBlender:
    def test_unavailable_raises(self, tmp_path: Path) -> None:
        with patch(
            "demodsl_blender.bridge.check_blender_available",
            return_value=False,
        ):
    ...
    def test_successful_render(self, tmp_path: Path) -> None:
        ...
        with (
            ...
            patch("demodsl_blender.bridge.subprocess.run", side_effect=fake_run),
        ):
    ...
    def test_nonzero_exit_raises(self, tmp_path: Path) -> None:
        ...
        with (
            ...
            patch(
                "demodsl_blender.bridge.subprocess.run",
    ...
    def test_temp_params_cleaned_up(self, tmp_path: Path) -> None:
        ...
        with (
            ...
            patch("demodsl_blender.bridge.subprocess.run", side_effect=spy_run),
        ):
...
class TestHeadlessBlenderProvider:
    def test_registered_in_factory(self) -> None:
        import demodsl_blender.provider  # noqa: F401 — registration side effect
    ...
    def test_render_delegates_to_bridge(self, tmp_path: Path) -> None:
        import demodsl_blender.provider  # noqa: F401
        ...
        with patch(
            "demodsl_blender.provider.render_via_blender",
            return_value=output,
        ) as mock_render:
...
class TestEngineDeviceRendering:
    def test_apply_device_rendering_fallback_when_unavailable(
        ...
        with patch(
            "demodsl_blender.provider.check_blender_available",
            return_value=False,
        ):
