...
class Transitions(_StrictBase):
    ...
    #: ``type`` → ffmpeg ``xfade`` transition name. ``crossfade``/``slide``/
    #: ``zoom`` are friendly aliases kept for backward compatibility; every
    #: other value is a real ffmpeg ``xfade`` transition name (see
    #: ``ffmpeg -h filter=xfade``) and maps to itself.
